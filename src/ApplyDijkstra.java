...
public class ApplyDijkstra {
	public static void main (String[] args) {
		...
		//Make sure there was no error from ApplyBellmanFord arguments
		...
		if (args.length < 1) {
			StdOut.println("Must provide argument for Dijkstra Shortest Path");
			System.exit(0);
		}
		...
		DijkstraSP sp = new DijkstraSP(gDoublePrime, s);
        ...
        */
		...
		for (int t = 0; t < gDoublePrime.V(); t++) {
            if (sp.hasPathTo(t)) {
                ...
                StdOut.printf("%d to %d (%.2f)  ", s, t, distTo);             //print distance of path (distTo)
                if (sp.hasPathTo(t)) {
                    for (DirectedEdge e : sp.pathTo(t)) {
                    	for (DirectedEdge ee : originalG.edges()) {
                    		if (e.to() == ee.to() && e.from() == ee.from()) {
		...
		StdOut.println("\nNovacky Shortest Path Running time: E*V + E*logV = E(V + logV)--> reduces to E*V");
	}
}
